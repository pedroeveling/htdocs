<?php
      ...
      //$query= "select * from $variavel s inner join funcionario f on (s.coordenador = f.id) where f.cargo = 1";
	   ...
	   $query= "select c.*, f.nome from $variavel c inner join funcionario f on (f.id = c.funcionario)";
       $consulta = mysqli_query($conexao, $query);
	   $r_final= "<b> Caixas: </b>";
       $r_final.= "<select id=";
       $r_final.="'setores' ";
       // chama e envia o latitude e longitude selecionado para a  função Dados2
       $r_final.="onchange='Dados2(this.value)'>";
       $r_final.="<option value='Escolha um caixa'> Escolha um dos caixas</option>";
