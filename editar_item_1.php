<?php
      ...
      //$query= "select * from $variavel s inner join funcionario f on (s.coordenador = f.id) where f.cargo = 1";
	   ...
	   $query= "select i.*,p.nome from item i inner join produto p on (i.produto = p.id)";
       $consulta = mysqli_query($conexao, $query);
	   $r_final= "<b> Item: </b>";
       $r_final.= "<select id=";
       $r_final.="'setores' ";
       // chama e envia o latitude e longitude selecionado para a  função Dados2
       $r_final.="onchange='Dados14(this.value)'>";
       $r_final.="<option value='Escolha um cliente'> Escolha um item</option>";
