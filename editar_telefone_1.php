<?php
      ...
      //$query= "select * from $variavel s inner join funcionario f on (s.coordenador = f.id) where f.cargo = 1";
	   ...
	   $r_final= "<b> Número Telefone: </b>";
       $r_final.= "<select id=";
       $r_final.="'setores' ";
       // chama e envia o latitude e longitude selecionado para a  função Dados2
       $r_final.="onchange='Dados13(this.value)'>";
       $r_final.="<option value='Escolha um cliente'> Escolha um número de telefone</option>";
        
	   while($result = mysqli_fetch_array($consulta))
		{	
			$id = $result['id'];
			$descricao = $result['numero'];
